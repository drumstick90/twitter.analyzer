Market Signal Extraction System — Prompt Suite (v2)
Overview
Two-layer system for extracting and synthesising market positioning signals from financial commentators on social media.

Extraction Agent — runs once per commentator per 24h cycle. Scenario-agnostic.
Orchestrator Agent — receives 4–5 extraction reports, synthesises into a positioning plan.


PROMPT 1: Extraction Agent
<system>
You are a financial market signal extraction agent. You receive a batch of social media posts (typically 24h) from a single commentator. Your job is to extract every actionable market view, positioning signal, and risk flag — regardless of asset class, geography, or market regime.
...
<user>
Here is today's post batch for commentator {{HANDLE}}:
...
PROMPT 2: Orchestrator Agent
<system>
You are a portfolio positioning orchestrator. You receive structured signal extraction reports from multiple financial commentators (typically 4–5, produced by upstream extraction agents). Your job is to synthesise these into a coherent daily positioning plan.
...

{
  ...
  "positioning_plan": {
    ...
    "core_positions": [
      {
        ...
        "tickers": ["<CL>", "<BZ>", "<USO>", "<UCO>", "<SPY>", ...],
        "option_spec": "<when options: calls/puts/spreads, strike, expiry. Null if spot.>",
    ...
    "convex_bets": [
      {
        "instrument_or_theme": "<specific theme>",
        "tickers": ["<CL>", "<BZ>", "<SPY>", "<QQQ>", "<HYG>", ...],
        "option_spec": "<calls/puts/spreads, strike, expiry. Null if not options.>",
        "direction": "<long|short>",
        "scenario_required": "<what needs to happen>",
        "loss_if_wrong": "<characterise>",
        "gain_if_right": "<characterise>",
        "supporting_sources": [...],
    ...
    "explicit_avoids": [
      {
        ...
        "tickers": ["<CL>", "<BZ>", "<ES>", "<NQ>", ...],
        "why": "<reasoning>",
        "fragility_type": "<crowded|binary_event|...>"
      }
    ]
  },
...
<user>
Today's extraction reports:

{{EXTRACTION_JSONS}}
</user>

Pipeline Architecture
[Scraper] → raw posts per commentator (24h batch)
    ↓
[Extraction Agent] × N (parallel, one per commentator)
    ↓  each outputs a JSON extraction report
[Orchestrator Agent] (single run, ingests all extractions)
    ↓
[Output] → daily positioning plan JSON
    ↓ (optional)
[Formatter] → human-readable briefing (markdown, email, Slack)
Model Recommendations
AgentModelReasoningExtractionclaude-sonnet-4-20250514Fast, cheap, strong at structured extractionOrchestratorclaude-opus-4-6Deeper reasoning for cross-source synthesis
Commentator Selection Criteria
Select for:

Domain diversity — at least 2-3 distinct primary domains across your panel
Independence — avoid people in the same reply threads constantly
Signal density — prefer commentators who express views, not just share links
Falsifiability — prefer commentators who make specific, testable claims

Scraper Requirements
Critical fields to capture per post:

Full text (including emoji)
Timestamp
Engagement metrics (likes, retweets/reposts, replies, views)
Parent post text (for replies and quote posts) — this is essential, half the signal is in what they're reacting to
URLs and media flags (even if unresolvable)
Community notes or corrections if present
#!/usr/bin/env python3
"""
Analysis script for @NTFabiano's 2024 tweets focusing on likes trends and engagement patterns.
"""
...
class NTFabiano2024Analyzer:
    ...
    def load_2024_data(self):
        """Load all 2024 tweet data from JSON files."""
        print("🔍 Loading 2024 tweet data...")
        
        # Find all 2024 files
        files = [f for f in os.listdir(self.data_dir) if f.startswith("tweets_NTFabiano_2024_page_")]
    ...
    def analyze_likes_trends(self):
        ...
        print("\n" + "="*60)
        print("📈 LIKES TREND ANALYSIS FOR 2024")
        print("="*60)
    ...
    def analyze_suspicious_patterns(self):
        """Analyze patterns that might indicate automation, bot activity, or purchased engagement."""
        print("\n" + "="*60)
        ...
        print("🔍 Analyzing patterns for potential automation or artificial growth...")
        
        # 1. Timing patterns analysis
        print(f"\n⏰ TIMING PATTERNS ANALYSIS:")
        
        # Check for exact time patterns
        exact_times = self.df.groupby('hour')['created_at'].count()
        suspicious_hours = exact_times[exact_times > 100]  # More than 100 tweets in same hour
        
        if len(suspicious_hours) > 0:
            print(f"  ⚠️  SUSPICIOUS: {len(suspicious_hours)} hours with >100 tweets:")
            for hour, count in suspicious_hours.items():
                print(f"    {hour}:00 - {count} tweets")
        else:
            print(f"  ✅ No suspicious hour clustering detected")
        
        # Check for consistent posting intervals
        self.df_sorted = self.df.sort_values('created_at').copy()
        self.df_sorted['time_diff'] = self.df_sorted['created_at'].diff()
        
        # Find tweets posted within very short intervals
        short_intervals = self.df_sorted[self.df_sorted['time_diff'] < pd.Timedelta(minutes=5)]
        if len(short_intervals) > 0:
            print(f"  ⚠️  SUSPICIOUS: {len(short_intervals)} tweets posted within 5 minutes of each other")
            print(f"    First few examples:")
            for i, (_, tweet) in enumerate(short_intervals.head(5).iterrows()):
                print(f"      {tweet['created_at'].strftime('%Y-%m-%d %H:%M:%S')} - {tweet['text'][:50]}...")
        else:
            print(f"  ✅ No suspicious rapid-fire posting detected")
        
        # 2. Engagement patterns analysis
        print(f"\n📊 ENGAGEMENT PATTERNS ANALYSIS:")
        
        # Check for suspicious engagement ratios
        high_engagement_tweets = self.df[self.df['likes'] > 1000]
        if len(high_engagement_tweets) > 0:
            print(f"  📈 High engagement tweets (>1000 likes): {len(high_engagement_tweets)}")
            
            # Check views vs likes ratio for high engagement tweets
            for _, tweet in high_engagement_tweets.head(5).iterrows():
                views = tweet.get('views', 0)
                likes = tweet.get('likes', 0)
                if views > 0:
                    ratio = (likes / views) * 100
                    print(f"    Tweet: {tweet['text'][:50]}...")
                    print(f"      Likes: {likes:,} | Views: {views:,} | Ratio: {ratio:.2f}%")
                    if ratio > 10:  # Suspicious if >10% like/view ratio
                        print(f"      ⚠️  SUSPICIOUS: Very high like/view ratio!")
        
        # 3. Content pattern analysis
        print(f"\n📝 CONTENT PATTERN ANALYSIS:")
        
        # Check for repetitive content patterns
        text_samples = self.df['text'].head(100).str.lower()
        common_patterns = []
        
        # Look for common hashtags, phrases, or patterns
        hashtag_pattern = r'#\w+'
        hashtags = []
        ...
        suspicious_phrases = [(phrase, count) for phrase, count in phrase_counts.items() if count > 3]
        
        if suspicious_phrases:
            print(f"  ⚠️  SUSPICIOUS: Repetitive phrases detected:")
            for phrase, count in suspicious_phrases[:5]:
                print(f"    '{phrase}': {count} times")
        
        # 4. Source analysis
        print(f"\n🔧 SOURCE ANALYSIS:")
        source_counts = self.df['source'].value_counts()
        print(f"  📱 Tweet sources:")
        for source, count in source_counts.head(10).items():
            print(f"    {source}: {count} tweets")
            
            # Check if source might indicate automation
            if any(auto_indicator in source.lower() for auto_indicator in ['buffer', 'hootsuite', 'ifttt', 'zapier', 'socialflow']):
                print(f"      ⚠️  SUSPICIOUS: Potential automation tool detected!")
        
        # 5. Growth timeline analysis
        print(f"\n📈 GROWTH TIMELINE ANALYSIS:")
        
        # Analyze likes growth over time
        monthly_growth = self.df.groupby(self.df['created_at'].dt.to_period('M'))['likes'].sum()
        
        print(f"  📅 Monthly likes progression:")
        for period, total_likes in monthly_growth.items():
            print(f"    {period}: {total_likes:,} likes")
        
        # Check for sudden spikes
        if len(monthly_growth) > 1:
            growth_rates = []
            for i in range(1, len(monthly_growth)):
                current = list(monthly_growth.values())[i]
                previous = list(monthly_growth.values())[i-1]
                if previous > 0:
                    growth_rate = ((current - previous) / previous) * 100
                    growth_rates.append(growth_rate)
                    print(f"    Growth rate {list(monthly_growth.keys())[i-1]} → {list(monthly_growth.keys())[i]}: {growth_rate:+.1f}%")
                    
                    if growth_rate > 1000:  # Suspicious if >1000% growth
                        print(f"      ⚠️  SUSPICIOUS: Extremely high growth rate!")
        
        # 6. Summary of suspicious indicators
        ...
        if suspicious_count == 0:
            ...
            print(f"  📊 Growth appears organic based on available data")
        else:
        ...
        print(f"\n💡 RECOMMENDATIONS:")
        if suspicious_count > 0:
            print(f"  • Check for bot networks or purchased engagement")
            print(f"  • Analyze follower growth patterns")
            print(f"  • Monitor for coordinated retweet/like campaigns")
            print(f"  • Verify content authenticity and sources")
        else:
            print(f"  • Growth appears organic - likely viral content")
            print(f"  • Check for major media coverage or influencer mentions")
            print(f"  • Analyze content quality and engagement strategies")
    ...
    def create_visualizations(self):
        ...
        fig.suptitle('@NTFabiano 2024 Twitter Engagement Analysis', fontsize=16, fontweight='bold')
        ...
        # Save the plot
        output_file = os.path.join(self.data_dir, 'ntfabiano_2024_likes_analysis.png')
    ...
    def create_timeline_chart(self):
        ...
        # Customize the chart
        plt.title('@NTFabiano 2024: Chronological Timeline of Tweet Likes', 
...
Best Month: {timeline_df.groupby(timeline_df['created_at'].dt.month)['likes'].sum().idxmax()}"""
        ...
        timeline_file = os.path.join(self.data_dir, 'ntfabiano_2024_timeline_likes.png')
    ...
    def save_analysis_report(self):
        ...
        # Convert top tweets to serializable format
        top_tweets_df = self.df.nlargest(10, 'likes')[['id', 'created_at', 'text', 'likes']]
        for _, tweet in top_tweets_df.iterrows():
        ...
        # Save report
        output_file = os.path.join(self.data_dir, 'ntfabiano_2024_likes_analysis_report.json')
    ...
    def run_full_analysis(self):
        ...
        print("🚀 Starting @NTFabiano 2024 Likes Trend Analysis...")
        print("="*60)
        
        # Load and process data
        self.load_2024_data()
        if len(self.tweets_2024) == 0:
            print("❌ No 2024 data found. Please run the scraper first for 2024.")
            return
        ...
        self.analyze_suspicious_patterns()  # Add suspicious patterns analysis
...
def main():
    ...
    analyzer = NTFabiano2024Analyzer()
